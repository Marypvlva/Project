...
class PhantomSDK:
    ...
    def __init__(self, sdk_dir: Path) -> None:
        ...
        if not self.sdk_dir.is_dir():
            raise FileNotFoundError(f"Phantom SDK directory not found: {self.sdk_dir}")
    ...
    def camera_count(self) -> int:
        ...
        hr = self.phcon.PhGetCameraCount(ctypes.byref(count))
        self.check_hr(hr, "PhGetCameraCount")
        return int(count.value)

    def wait_for_cameras(self, timeout_seconds: float) -> int:
    ...
    def camera_identity(self, camera_index: int) -> tuple[int, str]:
        ...
        hr = self.phcon.PhGetCameraID(camera_index, ctypes.byref(serial), name)
        self.check_hr(hr, "PhGetCameraID")
        return int(serial.value), name.value.decode("utf-8", errors="replace").strip()

    def open_live_cine(self, camera_index: int) -> tuple[ctypes.c_void_p, int]:
        ...
        try:
            hr = self.phfile.PhSetUseCase(handle, PHANTOM_UC_VIEW)
    ...
    def destroy_cine(self, handle: ctypes.c_void_p | None) -> None:
...
def phantom_buffer_to_bgr(
    ...
    *,
    ...
    if required > max_buffer_size:
        raise RuntimeError(
            f"Phantom image needs {required} bytes, buffer has {max_buffer_size}"
        )

    raw = np.ctypeslib.as_array(buffer, shape=(max_buffer_size,))
    rows = raw[:required].reshape(height, stride)[:, :row_bytes]
...
class PhantomLatestFrameCapture:
    ...
    def __init__(
        ...
        *,
        ...
        if camera_timeout_seconds <= 0:
            raise ValueError("--camera-timeout must be > 0")
        if max_consecutive_failures <= 0:
            raise ValueError("max_consecutive_failures must be > 0")
    ...
    def start(self) -> "PhantomLatestFrameCapture":
        try:
            self.sdk.register()
            ...
            self._cine_handle, self._max_image_size = self.sdk.open_live_cine(
                self.camera_index
            )
    ...
    def _reader_loop(self) -> None:
        failures = 0
        try:
            if self._cine_handle is None or self._image_buffer is None:
                    ...
                    if failures >= self.max_consecutive_failures:
                        with self._lock:
                            self._error = (
                                ...
                                f"{self.sdk.error_text(int(hr))} (HRESULT={int(hr)})"
                            )
                        self._stop_event.set()
                        break
                    time.sleep(0.005)
    ...
    def _cleanup(self) -> None:
        if self._cine_handle is not None:
            try:
                self.sdk.destroy_cine(self._cine_handle)
            except Exception:
                pass
...
def create_frame_source(args: argparse.Namespace):
...
def load_normalization(
    ...
    if mean is None or std is None:
        raise ValueError(
            "laser_mean/laser_std are missing. Use a checkpoint produced by the "
            "current train_transformer.py or pass --norm-json logs/transformer_norm.json"
        )
...
def normalize_laser_params(
    ...
    if not np.all(np.isfinite(normalized)):
        raise FloatingPointError("Normalized laser parameters contain NaN/inf")
...
def build_temporal_input(
    ...
    live_tensor = torch.stack(list(frame_buffer), dim=0)
    expected_live = (live_count, 3, image_size, image_size)
    if tuple(live_tensor.shape) != expected_live:
        raise RuntimeError(
...
def prediction_text(prediction: float | None, censor_threshold: float) -> str:
    ...
    if prediction < 0:
        return f"Final R: {prediction:.2f} kOhm/sq [WARN < 0]"
    if prediction >= censor_threshold:
        return f"Final R: >= {censor_threshold:.1f} kOhm/sq"
    return f"Final R: {prediction:.2f} kOhm/sq"
...
def run_live_inference(args: argparse.Namespace) -> None:
    ...
    model = load_temporal_regressor(checkpoint_path, map_location="cpu")
    model.to(device)
    model.eval()

    window_size = int(model.window_size)
    if window_size <= 0:
        raise ValueError(f"Invalid model window_size: {window_size}")

    image_size = int(config.get("img_size", 128))
    if image_size <= 0:
        raise ValueError(f"Invalid checkpoint img_size: {image_size}")

    checkpoint_position_step = config.get("position_step")
    if checkpoint_position_step is None:
        ...
        position_step = float(checkpoint_position_step)
        if args.position_step is not None:
    ...
    if not math.isfinite(position_step) or not 0.0 < position_step < 1.0:
    ...
    censor_threshold = float(
        config.get("censor_threshold_kOhm", DEFAULT_CENSOR_THRESHOLD_KOHM)
    )
    if not math.isfinite(censor_threshold) or censor_threshold <= 0:
        raise ValueError(f"Invalid censor threshold: {censor_threshold}")
    ...
    print(f"Process duration:     {args.duration_seconds:.3f} s")
    print(f"ML sampling interval: {sampling_interval_seconds:.4f} s")
    print(
        "Raw params:           "
        f"[{args.power_mw}, {args.speed_mm_s}, {args.distance_um}]"
    )
    print("Param order:           [power_mW, speed_mm_s, distance_um]")
    print(
        "Normalized params:    "
        ...
        while True:
            if frame_source.is_stopped():
                if frame_source.error:
                    raise RuntimeError(frame_source.error)
                raise RuntimeError("Live capture thread stopped unexpectedly")

            latest = frame_source.get_latest(copy=True)
            if latest is None:
                time.sleep(0.001)
                continue

            raw_frame, _capture_timestamp, capture_sequence = latest
            now = time.monotonic()
            ...
            if now < process_start_time:
                ...
                if next_grid_index < len(position_grid):
                    ...
                    if (
                        ...
                        position_lag = progress - target_position
                        ...
                        prediction = predict_final_resistance(
                            model=model,
                            frames=frames,
                        ...
                        last_model_position = target_position
                        last_sampled_sequence = capture_sequence
                        ...
                        if inference_ms / 1000.0 > sampling_interval_seconds:
                            print(
            ...
            if now >= next_preview_time:
                display_frame = draw_overlay(
                    ...
                    model_position=last_model_position,
                    window_count=len(frame_buffer),
                    window_size=window_size,
                ...
                if gui_enabled:
                    try:
                        cv2.imshow("LIG Live Final Resistance", display_frame)
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if not math.isfinite(args.start_delay_seconds) or args.start_delay_seconds < 0:
        raise SystemExit("--start-delay-seconds must be finite and >= 0")

    for name, value in [
        ("--power-mw", args.power_mw),
        ("--speed-mm-s", args.speed_mm_s),
        ("--distance-um", args.distance_um),
    ]:
        if not math.isfinite(value):
